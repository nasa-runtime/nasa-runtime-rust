...
impl SignalBroker {
    /// 业务作用：按指定模式建立信号控制面，并等待平台 handler 完成注册。
    ///
    ...
    /// 业务作用：等待下一次已注册信号；关闭模式下永久 pending，不制造虚假事件。
    ///
    /// # 参数
    ///
...
/// 业务作用：根据公开状态把信号分流为正常 Runner 事件或停机中立即强退。
///
/// # 参数
///
