...
impl fmt::Display for SecretResolveError {
    /// 输出 secret ID 与结构化原因，永不包含解析出的敏感值。
...
impl From<FragmentConfig> for SecretFragmentRef {
    /// 将配置 fragment 标签转换为解析器使用的路径、环境变量或文件引用。
...
/// 从原始树的 `secrets` 段解析出全部 [`SecretSpec`];无 `secrets` 段时返回空。
...
/// 将 secret ID 限制为短 ASCII 标识，避免路径歧义、控制字符与无界诊断字段。
...
/// 按点分路径在树中取**标量字符串**;非字符串或不存在返回 `None`(禁止引用子树/递归 spec)。
...
/// 把点分路径处的标量替换为 `<redacted>`;路径不存在则忽略(只脱敏已解析成功的 fragment)。
...
/// 对原始候选树求私有 fingerprint(canonical JSON 文本的 hash);仅供无变化判断,不对外。
///
...
/// 从原始候选树解析 secret、脱敏 fragment、求候选 fingerprint(同一 generation)。
///
/// # 参数
///
