...
use crate::{
    ...
    ParticipantHandled, ParticipantRuntime, ResultDeliveryDisposition, ResultDeliveryPolicy,
    SagaCommandEnvelope, SagaCommandProcessingError, SagaCommandService, SagaResultEnvelope,
    COMMAND_EVENT_TYPE, RESULT_EVENT_TYPE,
};
...
/// 做显式类型化路由，并继续复用同一个 [`SagaKafkaCommandConsumer`]。
...
impl<H: SagaCommandHandler> SagaKafkaCommandConsumer<H> {
    ...
    pub async fn handle_delivery(
        ...
        let result = self
            .handler
            .handle_authenticated_command(envelope, route.producer())
            .await;
        record_kafka_command_processing(started_at.elapsed());
        match result {
...
impl<H: SagaResultHandler> SagaKafkaResultConsumer<H> {
    ...
    pub async fn handle_delivery(
        ...
        let result = self
            .handler
            .handle_authenticated_result(envelope, producer, now_ms)
            .await;
        record_kafka_processing(started_at.elapsed());
        match result {
            Ok(outcome) => Ok(outcome),
