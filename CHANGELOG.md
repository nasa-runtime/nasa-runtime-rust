...
- 顺序边界收窄到「分区 + 任务类型」：严格类型同分区内按提交顺序串行，非严格类型跨分区并发；
  空闲分区经一次执行权 CAS 接管热点严格 lane，不移动队列数据，兼容入口语义不变。
- 类型化入口提供任务句柄（状态查询、取消、等待终态）与延迟提交；零延迟等价于立即提交；取消
  到达终态即归还全局在飞名额。计数按受理与发生时点生效（延迟登记即计入受理，到期拒绝计入
  对应拒绝类别），排空后公开累计量守恒。
- 双层准入（每 lane 深度上限 + 全局在飞预算）之外，类型化 lane 总数受配置上限约束，动态任务
  类型不能无界扩张注册表与指标序列，超限提交得到明确拒绝；兼容入口保留 lane 豁免上限，不因
  类型化负载先到先得而失去容量。
- 停机由独立受监督任务驱动：调用方取消不影响推进；返回以 worker、在途任务、全部延迟定时器
  （含已取消的）的退出证明与 producer 离场为界；强制中止与冻结两类损耗全部计入停机报告、
  指标与证据容器。
