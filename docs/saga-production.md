...
Kafka 至少提供 command、result 和 DLT 三类 topic：
...
6. 消费循环、Outbox dispatcher 和 timer poller 就绪后，最后开放业务路由。
...
- Kafka 正常流量、retry storm、DLT 积压和 broker 故障后的追赶速率；
...
刷新 Outbox 与指标，最后关闭 Kafka 和数据库连接。超出停机预算时保留 durable 事实，由新副本接管，
不应强制确认未完成消息。

灾难恢复需要保存 MySQL point-in-time recovery、Kafka offset 与 topic 保留策略、流程定义快照、ACL、
